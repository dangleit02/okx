...
export class OkxService {
    ...
    async cancelAllTypeOfOpenOrdersForOneCoin(coin: string, side: 'buy' | 'sell' | null = null, onlyForDown: boolean = false) {
        // const res1 = await this.cancelOpenOrdersForOneCoin(coin, side, onlyForDown);
        const res2 = await this.cancelOpenConditionalOrdersForOneCoin(coin, side, onlyForDown);
        return { res2 };
    }

    cancelAllTypeOfOpenOrders(side: 'buy' | 'sell' | null = null) {
        return this.cancelAllOpenConditionalOrders(side);
    }

    async cancelOpenConditionalOrdersForOneCoin(coin: string, side: 'buy' | 'sell' | null = null, onlyForDown: boolean = false) {
    ...
    async cancelAllOpenConditionalOrders(side: 'buy' | 'sell' | null = null) {
        const timestamp = new Date().toISOString();
    ...
    async buyOneCoin(isTesting: boolean, removeExistingBuyOrders: string, coin: string, results: any[], addBuySurprisePrice: string, numberOfUSDT: number, autobuy: string) {
        if (!isTesting) {
            if (removeExistingBuyOrders === 'true') {
                const res1 = await this.cancelAllTypeOfOpenOrdersForOneCoin(coin, 'buy');
        ...
        //   const res2 = await this.okxService.placeMultipleBuyOrdersFromMinPriceToMaxPriceForUp(coin, isTesting);
        ...
        if (addBuySurprisePrice === 'true') {
            const res3 = await this.placeSurprisePriceBuyOrder(coin, numberOfUSDT, isTesting);
    ...
    async sellOneCoin(isTesting: boolean, removeExistingSellOrders: string, coin: string, results: any[], addSellSurprisePrice: string, addSellStopLoss: string, addSellTakeProfit: string, onlyForDown: string, justOneOrder: string) {
        if (!isTesting) {
            if (removeExistingSellOrders === 'true') {
                const res1 = await this.cancelAllTypeOfOpenOrdersForOneCoin(coin, 'sell', onlyForDown === 'true');
    ...
    // async cancelOpenOrdersForOneCoin(coin: string, side: 'buy' | 'sell' | null = null, onlyForDown: boolean = false) {
    //     const timestamp = new Date().toISOString();
    ...
    // async cancelAllOpenOrders(side: 'buy' | 'sell' | null = null) {
    //     const timestamp = new Date().toISOString();
    ...
    // async placeMultipleBuyOrdersFromMinPriceToMaxPriceForUp(coin: string, testing: boolean = true) {
    ...
    // async placeBuyOrdersForAllCoinsForBuyFromMinPriceToMaxPriceForUp(testing: boolean = true) {
    ...
    //         const res = await this.placeMultipleBuyOrdersFromMinPriceToMaxPriceForUp(coin, testing);
    ...
    // async placeMultipleBuyOrdersFromMinPriceToMaxPrice(coin: string, testing: boolean = true) {
    //     this.logger.log(`Starting to place multiple orders for ${coin}, testing mode: ${testing}`);
    //     const maxUsdt = this.config.get<number>('maxUsdt');
    //     const riskPerTrade = this.config.get<number>('riskPerTrade');
    //     const amountOfUsdtPerStep = this.config.get<number>('amountOfUsdtPerStep');
